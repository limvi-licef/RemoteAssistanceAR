...
public class SpeakerController
{
    ...
    public async void Start()
    {
        ...
        try
        {
            ...
            m_sPortLocalUDP = "62511";
            m_sIPRemoteUDP = "10.44.161.15";
            m_sPortRemoteUDP = "62510";
            ...
            await m_dsUdpSocket.BindEndpointAsync(new HostName(m_sIPLocalUDP), m_sPortLocalUDP);
        ...
        /*BinaryWriter writer; = await new BinaryWriter(File.Open("kikou.txt", FileMode.Create));
   ...
   /* public void OnMessageReceived(NetworkConnection connection, NetworkInMessage message)
    ...
    private void AddProminentSpeaker(UInt32 sourceID, float averageAmplitude, float posX, float posY, float posZ)
    {
        if (this.prominentSpeakerCount < AudioConstants.MaximumProminentSpeakers)
        {
            ...
            prominentSpeakerInfo.hrtfPosition[0] = posX;
            prominentSpeakerInfo.hrtfPosition[1] = posY;
        }
    }
        ...
        Windows.Networking.Sockets.DatagramSocketMessageReceivedEventArgs args)
    {
        ...
        m_fsWav.Write(message, 0, message.Length);
        Debug.Write("[Playback::Socket_MessageReceived] ");
        ...
        if (temp != "")
        {
            Debug.WriteLine("[SpeakerController::Socket_MessageReceived] Received data are not empty!!! : " + temp + "\n");
        }

        internalStore(message, 0, message.Length);

        //byte headerSize = message.ReadByte();
        int messagePos = 0;
        byte headerSize = message[messagePos];

        //Int32 pack = message.ReadInt32();
        Int32 pack = BitConverter.ToInt32(message, messagePos);
        messagePos += 4;
        ...
        {
            //extendedSampleRate = message.ReadInt32();
            extendedSampleRate = BitConverter.ToInt32(message, messagePos);
            messagePos += 4;
        }
        ...
        {
            //float averageAmplitude = message.ReadFloat();
            float averageAmplitude = BitConverter.ToSingle(message, messagePos);
            messagePos += 4;
            //UInt32 hrtfSourceID = (UInt32)message.ReadInt32();
            UInt32 hrtfSourceID = (UInt32)BitConverter.ToInt32(message, messagePos);
            messagePos += 4;
            //Vector3 hrtfPosition = new Vector3();
            //Vector3 hrtfDirection = new Vector3();
            float[] hrtfPosition = new float[3];
            float[] hrtfDirection = new float[3];
            if (hrtfSourceID != 0)
            {
                /*hrtfPosition.x = message.ReadFloat();
                hrtfPosition.y = message.ReadFloat();
                ...
                hrtfDirection.y = message.ReadFloat();
                hrtfDirection.z = message.ReadFloat();*/
                hrtfPosition[0] = BitConverter.ToSingle(message, messagePos);
                messagePos += 4;
                hrtfPosition[1] = BitConverter.ToSingle(message, messagePos);
                messagePos += 4;
                hrtfPosition[2] = BitConverter.ToSingle(message, messagePos);
                messagePos += 4;

                hrtfDirection[0] = BitConverter.ToSingle(message, messagePos);
                messagePos += 4;
                hrtfDirection[1] = BitConverter.ToSingle(message, messagePos);
                messagePos += 4;
                hrtfDirection[2] = BitConverter.ToSingle(message, messagePos);
                messagePos += 4;
            ...
            for (int j = 0; j < channelCount; j++)
            {
                ...
                if (codecType != 0)
                {
                    // if compressed, size is first 2 bytes, sampleCount should be number of bytes after decompression
                    //size = message.ReadInt16();
                    size = BitConverter.ToInt16(message, messagePos);
                    messagePos += 2;
                }
                ...
                //message.ReadArray(this.networkPacketBufferBytes, (uint)(size * bytesPerSample));
                message.CopyTo(this.networkPacketBufferBytes, (size * bytesPerSample));
